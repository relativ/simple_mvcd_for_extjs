...
var
  FCikisIrsaliyesi: TFCikisIrsaliyesi;
...
Begin
        Result := FormatFloat(DMirsaliye.CikisBodroIBQCikisTUTAR.DisplayFormat, x);
end;

Procedure Datalari_Ac;
Begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Active = false then
        begin
                DMirsaliye.CikisIrsaliyesiIBQ.Open;
                DMirsaliye.CikisBodroIBQ.Open;
                DMirsaliye.CAltUstBilgiIBQ.Open;
...
procedure TFCikisIrsaliyesi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
        DMirsaliye.CikisIrsaliyesiIBQ.Close;
        DMirsaliye.CikisBodroIBQ.Close;
        Action := caFree;
end;

procedure TFCikisIrsaliyesi.BFirstPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMirsaliye.CikisIrsaliyesiIBQ.First;
end;

procedure TFCikisIrsaliyesi.ButtonPreviousClick(Sender: TObject);
begin
        Datalari_Ac;
        DMirsaliye.CikisIrsaliyesiIBQ.Prior;
end;

procedure TFCikisIrsaliyesi.ButtonNextClick(Sender: TObject);
begin
        Datalari_Ac;
        DMirsaliye.CikisIrsaliyesiIBQ.Next;
end;

procedure TFCikisIrsaliyesi.BLastPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMirsaliye.CikisIrsaliyesiIBQ.Last;
end;

procedure TFCikisIrsaliyesi.BitBtn1Click(Sender: TObject);
begin
        PageControl1.ActivePage := TabSheet1;
        DMirsaliye.CikisBodroIBQ.DisableControls;
        Datalari_Ac;
        DMirsaliye.CikisIrsaliyesiIBQ.Append;
        DMirsaliye.CikisIrsaliyesiIBQ.Post;
        BitBtn8.SetFocus;
        DMirsaliye.CikisBodroIBQ.EnableControls;
end;

procedure TFCikisIrsaliyesi.BitBtn2Click(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.RecordCount>0 Then
        begin
                if (DMirsaliye.CikisIrsaliyesiIBQBELGENO.AsString = '') or (DMirsaliye.CikisIrsaliyesiIBQBELGENO.AsFloat = 0) then
                begin
                        if Application.MessageBox('Bu Stok Ýþlemi silmek istediðinizden emin misiniz?',
                                '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
                        begin
                                DMirsaliye.CikisIrsaliyesiIBQ.Edit;
                                DMirsaliye.CikisIrsaliyesiIBQSISTEM.AsInteger := 2;
                                DMirsaliye.CikisIrsaliyesiIBQ.Post;
                        end;
...
procedure TFCikisIrsaliyesi.BitBtn3Click(Sender: TObject);
begin
        Close;
end;

procedure TFCikisIrsaliyesi.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TFCikisIrsaliyesi.DBOnlineEdit4Change(Sender: TObject);
begin
        GroupBox6.Caption := ' Dövizli Tutar ['+DMirsaliye.CikisIrsaliyesiIBQC_PARABIRIMI.AsString +' ] ';
end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
       if (DMirsaliye.CikisBodroIBQKAPATMA.AsInteger=2) and
                ((DMirsaliye.CikisBodroIBQCikisTUTAR.AsString<>'') or
                        (DMirsaliye.CikisBodroIBQCIKISTUTAR.AsString<>'') or
                        (DMirsaliye.CikisBodroIBQMIKTAR.AsString<>''))  Then
        Begin
                ...
                begin
                        ...
                        begin
                                if (Column.Field <> DMirsaliye.CikisBodroIBQCikisMIKTAR) and (Column.Field <> DMirsaliye.CikisBodroIBQCIKISMIKTAR) then
...
procedure TFCikisIrsaliyesi.Builemikapat1Click(Sender: TObject);
begin
        DMirsaliye.CikisBodroIBQ.edit;
        DMirsaliye.CikisBodroIBQKAPATMA.AsInteger:=2;
        DMirsaliye.CikisBodroIBQ.Post;
END;

procedure TFCikisIrsaliyesi.Busatilemia1Click(Sender: TObject);
begin
        DMirsaliye.CikisBodroIBQ.edit;
        DMirsaliye.CikisBodroIBQKAPATMA.AsInteger:=1;
        DMirsaliye.CikisBodroIBQ.Post;
end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1ContextPopup(Sender: TObject;
  MousePos: TPoint; var Handled: Boolean);
begin
        if DMirsaliye.CikisIrsaliyesiIBQCARIHESAPILISKI.AsString ='1' Then
        Begin
        if (DMirsaliye.CikisBodroIBQKAPATMA.AsInteger=1) Then
        Begin
        ...
        if (DMirsaliye.CikisBodroIBQCikisMIKTAR.AsFloat>0) or (DMirsaliye.CikisBodroIBQCikisTUTAR.AsFloat>0) Then
                SatrSil1.Visible:=True else SatrSil1.Visible:=False;

end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
        if (Key=#13) Then
        Begin
                if (DMirsaliye.CikisBodroIBQ.Modified=True ) Then
                        DMirsaliye.CikisBodroIBQ.Post;
                OnlineDBGrid1.EditorMode :=True;
                if BarkodluCikis = True Then
                Begin
                        BarkodluCikis := False;
                        DMirsaliye.CikisBodroIBQ.Append;
                end;
        end;
end;
...
procedure TFCikisIrsaliyesi.SatrEkle1Click(Sender: TObject);
begin
        DMirsaliye.CikisBodroIBQ.Append;
end;

procedure TFCikisIrsaliyesi.SatrSil1Click(Sender: TObject);
begin
        if DMirsaliye.CikisBodroIBQ.RecordCount>0 Then
        if Application.MessageBox('Bu satýrý silmek istediðinizden emin misiniz?',
                        '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
        Begin
                DMirsaliye.CikisBodroIBQ.Delete;
                if (DMirsaliye.CikisBodroIBQ.Modified=True ) Then
                        DMirsaliye.CikisBodroIBQ.Post;
        end;
end;

procedure TFCikisIrsaliyesi.BitBtn8Click(Sender: TObject);
begin
        if (DMirsaliye.CikisIrsaliyesiIBQ.Active) and (DMirsaliye.CikisBodroIBQ.Active) then
        begin
               FCariSecim.DataSet := DMirsaliye.CikisIrsaliyesiIBQ;
               FCariSecim.ShowModal;
        end else MessageBox(handle,'Yeni iþlem yapmadan cari seçemezsiniz.','Uyarý',MB_OK);
end;

procedure TFCikisIrsaliyesi.StokEkle1Click(Sender: TObject);
begin
                ...
                begin
                        ...
                        FFiyatSorgu.DataSet := DMirsaliye.CikisBodroIBQ;
...
procedure TFCikisIrsaliyesi.BitBtn4Click(Sender: TObject);
begin   {
if DMirsaliye.CikisIrsaliyesiIBQTAHSILAT.AsFloat>0 Then
Begin
        ...
        TQRLabel(FEvrak.Report.FindComponent('FaturaBasligi')).Caption := DMirsaliye.CikisIrsaliyesiIBQCARIADI.AsString +' '+DMirsaliye.CikisIrsaliyesiIBQCARISOYADI.AsString;
        if CariDM.CariKartIBQuery.Locate('CARIKOD',DMirsaliye.CikisIrsaliyesiIBQCARIKOD.AsString,[]) Then
        Begin
...
procedure TFCikisIrsaliyesi.BitBtn5Click(Sender: TObject);
begin             {
if DMirsaliye.CikisIrsaliyesiIBQBELGENO.AsInteger > 0 then
...
procedure TFCikisIrsaliyesi.HepsineStoaKaydet1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMirsaliye.CikisBodroIBQ.GetBookmark;
        DMProcedure.TumunuStokKaydetProc.ParamByName('ISLEMKOD_IN').AsInteger := DMirsaliye.CikisBodroIBQISLEMKOD.AsInteger;
        ...
        DMirsaliye.CikisBodroIBQ.Close;
        DMirsaliye.CikisBodroIBQ.Open;
        DMirsaliye.CikisBodroIBQ.GotoBookmark(yer);

end;

procedure TFCikisIrsaliyesi.HibiriniStoakeydetme1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMirsaliye.CikisBodroIBQ.GetBookmark;
        DMProcedure.TumuStokSilProc.ParamByName('ISLEMKOD_IN').AsInteger := DMirsaliye.CikisBodroIBQISLEMKOD.AsInteger;
...
procedure TFCikisIrsaliyesi.OnlineDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
        if Key = VK_F5 then
        begin
...
procedure TFCikisIrsaliyesi.lemSrasnaGreSrala1Click(Sender: TObject);
begin
        DMirsaliye.CikisBodroIBQ.SQL.Text := 'select * from IRSALIYE Where ISLEMKOD=:ISLEMKOD';
        DMirsaliye.CikisBodroIBQ.SQL.Add('Order by ISLEMKOD ASC');
        DMirsaliye.CikisBodroIBQ.Open;

end;

procedure TFCikisIrsaliyesi.FormDestroy(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
        DMirsaliye.CikisIrsaliyesiIBQ.Close;
        DMirsaliye.CikisBodroIBQ.Close;
end;

procedure TFCikisIrsaliyesi.CariHesaplikisi1Click(Sender: TObject);
var
        yer: pointer;
begin
...
procedure TFCikisIrsaliyesi.DBOnlineCombo1KeyPress(Sender: TObject; var Key: Char);
begin
        if Key = #13 then
        begin
                Key := #0;
...
procedure TFCikisIrsaliyesi.DBOnlineCombo1Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
        DMirsaliye.CikisIrsaliyesiIBQD_CikisTUTAR.DisplayFormat := '###,###.0 '+ DMirsaliye.CikisIrsaliyesiIBQC_CARIPARABIRIM.AsString;
//        DMirsaliye.CikisIrsaliyesiIBQD_TAHKDVTUTAR.DisplayFormat := '###,###.0 '+ DMirsaliye.CikisIrsaliyesiIBQPARABIRIMI.AsString;
//        DMirsaliye.CikisIrsaliyesiIBQD_TAHTUTAR.DisplayFormat := '###,###.0 '+ DMirsaliye.CikisIrsaliyesiIBQPARABIRIMI.AsString;
        GroupBox6.Caption := ' Dövizli Tutar ['+DMirsaliye.CikisIrsaliyesiIBQC_PARABIRIMI.AsString+']';
end;

procedure TFCikisIrsaliyesi.OnlineButton1Click(Sender: TObject);
begin
        ...
        FStokYeri.DataSet := DMirsaliye.CikisIrsaliyesiIBQ;
        FStokYeri.BitBtn3.Visible := true;
        FStokYeri.Show;
end;

procedure TFCikisIrsaliyesi.DBOnlineEdit15Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.OnlineButton2Click(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Active then
        begin
                ...
                DMSabit.GGSecimIBQ.SQL.Add('Order by GELGITADI');
                DMSabit.GGSecimIBQ.Open;
                FGelGidSec.DataSet := DMirsaliye.CikisIrsaliyesiIBQ;
                FGelGidSec.ShowModal;
        end;
end;

procedure TFCikisIrsaliyesi.DBOnlineEdit5Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.DBOnlineEdit7Change(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQC_CARIADI.AsString<>'' Then
                Panel2.Caption := 'Giriþ Ýrsaliyesi [ '+DMirsaliye.CikisIrsaliyesiIBQC_CARIADI.AsString + ' ]'
        else Panel2.Caption := 'Giriþ Ýrsaliyesi';
end;

procedure TFCikisIrsaliyesi.GroupBox3Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1Columns16KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMirsaliye.CikisIrsaliyesiIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade);
                        TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade := '';
                        DMirsaliye.CikisBodroIBQ.Edit;
                        DMirsaliye.CikisBodroIBQVADE.AsDateTime := Vade;
                        DMirsaliye.CikisBodroIBQ.Post;
                end;
        end;
...
procedure TFCikisIrsaliyesi.OnlineButton3Click(Sender: TObject);
begin
        if (DMirsaliye.CikisIrsaliyesiIBQCARIKOD.AsInteger > 0) or (DMirsaliye.CikisIrsaliyesiIBQCARINO.AsInteger > 0) then
        ...
        begin
                DMirsaliye.CikisIrsaliyesiIBQ.Edit;
                DMirsaliye.CikisIrsaliyesiIBQCARIKOD.AsInteger:=0;
                DMirsaliye.CikisIrsaliyesiIBQCARINO.AsInteger := 0;
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
        end;
end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1Columns7EditButtonClick(
  Column: TOnlineColumn);
begin
        FParaBirimSecim.DataSet := DMirsaliye.CikisBodroIBQ;
        FParaBirimSecim.ShowModal;
end;

procedure TFCikisIrsaliyesi.GroupBox1Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified Then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.OnlineDBCheckbox1Click(Sender: TObject);
begin
...
procedure TFCikisIrsaliyesi.DBOnlineLookupCombo1Click(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
        GroupBox6.Caption := ' Dövizli Tutar ['+DMirsaliye.CikisIrsaliyesiIBQC_PARABIRIMI.AsString +' ] ';
...
procedure TFCikisIrsaliyesi.DBOnlineLookupCombo1Exit(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Modified then
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.OnlineButton5Click(Sender: TObject);
var
        yer: pointer;
begin
        FKopyala.Show;
        DMProcedure.CeviriProc.ParamByName('ISLEMKOD_IN').AsInteger := DMirsaliye.CikisIrsaliyesiIBQISLEMKOD.AsInteger;
        ...
        DMirsaliye.CikisIrsaliyesiIBQ.Open;
        DMirsaliye.CikisBodroIBQ.Open;
        DMirsaliye.CikisIrsaliyesiIBQ.Locate('ISLEMKOD',DMProcedure.CeviriProc.ParamByName('ISLEMKOD3_IN').AsInteger,[]);
        DMData.IBTCariPro.CommitRetaining;
end;

procedure TFCikisIrsaliyesi.OnlineButton4Click(Sender: TObject);
begin
        if DMirsaliye.CikisIrsaliyesiIBQ.Active then
        begin
                DMSabit.PersonelIBQ.Open;
                FPersonelSec.DataSet := DMirsaliye.CikisIrsaliyesiIBQ;
...
procedure TFCikisIrsaliyesi.OnlineButton9Click(Sender: TObject);
var
        TopVade,TopFiyat,OrtVade: real;
begin
        if (DMirsaliye.CikisBodroIBQ.Active = true) and (DMirsaliye.CikisBodroIBQ.RecordCount > 0) then
        begin
                DMirsaliye.CikisBodroIBQ.First;
                while not DMirsaliye.CikisBodroIBQ.Eof do
                begin
                        TopVade := TopVade + (DMirsaliye.CikisBodroIBQCikisTUTAR.AsFloat * (DMirsaliye.CikisBodroIBQVADE.AsDateTime - DMirsaliye.CikisIrsaliyesiIBQTARIH.AsDateTime));
                        TopFiyat := TopFiyat + DMirsaliye.CikisBodroIBQCikisTUTAR.AsFloat;
                        DMirsaliye.CikisBodroIBQ.Next;
                end;
                OrtVade := TopVade / TopFiyat;
                OrtVade := OrtVade + (DMirsaliye.CikisIrsaliyesiIBQTARIH.AsFloat) ;
                DMirsaliye.CikisIrsaliyesiIBQ.Edit;
                DMirsaliye.CikisIrsaliyesiIBQVADE.AsString := DateToStr((OrtVade));
                DMirsaliye.CikisIrsaliyesiIBQ.Post;
        end;
...
begin
        ...
        DMirsaliye.CikisBodroIBQ.DisableControls;
        DMirsaliye.CikisBodroIBQ.First;
        for i := 0 to DMirsaliye.CikisBodroIBQ.RecordCount do
        begin
      //  while not DMirsaliye.CikisBodroIBQ.Eof do
       // begin
                ToplamVade := ToplamVade + DMirsaliye.CikisBodroIBQVADE.AsDateTime;
        ...
        DMirsaliye.CikisIrsaliyesiIBQ.Edit;
        DMirsaliye.CikisIrsaliyesiIBQVADE.AsString :=DateToStr(OrtVadeTarih);
        DMirsaliye.CikisIrsaliyesiIBQ.Post;

               }
end;

procedure TFCikisIrsaliyesi.OnlineDBGrid1Columns17KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMirsaliye.CikisBodroIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[17]).Vade);
...
procedure TFCikisIrsaliyesi.OnlineButton12Click(Sender: TObject);
begin
...
procedure TFCikisIrsaliyesi.OnlineButton13Click(Sender: TObject);
begin
        FCariSecim.DataSet := nil;
...
procedure TFCikisIrsaliyesi.OnlineButton14Click(Sender: TObject);
begin
...
procedure TFCikisIrsaliyesi.OnlineButton15Click(Sender: TObject);
begin
        FPersonelSec.DataSet := nil;
...
procedure TFCikisIrsaliyesi.OnlineButton10Click(Sender: TObject);
begin
        DMirsaliye.CikisIrsaliyesiIBQ.DisableControls;
        DMirsaliye.CikisIrsaliyesiIBQ.SQL.Text := 'select * from IRSALIYE where ISLEMTURKOD=43 AND SISTEM<>2';
        DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and TARIH >='+#39+DateToStr(DateTimePicker1.Date)+#39+' and TARIH <='+#39+DateToStr(DateTimePicker2.Date)+#39);
        if (OnlineEdit1.Text <> '') and (OnlineEdit2.Text <> '') then
                DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and IRALIYEISLEMNO >='+OnlineEdit1.Text+' and IRALIYEISLEMNO <='+OnlineEdit2.Text);
        if (OnlineEdit3.Text <> '') and (OnlineEdit4.Text <> '') then
                DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and CARINO >='+IntToStr(OnlineEdit3.Tag)+' and CARINO <='+IntToStr(OnlineEdit4.Tag));
        if (OnlineEdit5.Text <> '') and (OnlineEdit6.Text <> '') then
                DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and BELGENO >='+OnlineEdit5.Text+' and BELGENO <='+OnlineEdit6.Text);
        if (OnlineEdit7.Text <> '') and (OnlineEdit8.Text <> '') then
                DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and EVRAKNO >='+OnlineEdit7.Text+' and EVRAKNO <='+OnlineEdit8.Text);
        if (OnlineEdit9.Text <> '') and (OnlineEdit10.Text <> '') then
                DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('and PERSONELKOD >='+IntToStr(OnlineEdit9.Tag)+' and PERSONELKOD <='+IntToStr(OnlineEdit10.Tag));
        DMirsaliye.CikisIrsaliyesiIBQ.Open;
        DMirsaliye.CikisBodroIBQ.Open;
        DMirsaliye.CikisIrsaliyesiIBQ.EnableControls;
end;

procedure TFCikisIrsaliyesi.OnlineButton11Click(Sender: TObject);
begin
        OnlineEdit1.Text := '';
        OnlineEdit2.Text := '';
        OnlineEdit3.Text := '';
        ...
        OnlineEdit10.Tag := 0;
        DMirsaliye.CikisIrsaliyesiIBQ.DisableControls;
        DMirsaliye.CikisIrsaliyesiIBQ.SQL.Text := 'select * from IRSALIYE';
        DMirsaliye.CikisIrsaliyesiIBQ.SQL.Add('where ISLEMTURKOD =43 AND SISTEM<>2');
        DMirsaliye.CikisIrsaliyesiIBQ.Open;
        DMirsaliye.CikisIrsaliyesiIBQ.EnableControls;
        
end;

procedure TFCikisIrsaliyesi.DBRichEdit1Enter(Sender: TObject);
begin
        DMirsaliye.CAltUstBilgiIBQ.Edit;
end;

procedure TFCikisIrsaliyesi.DBRichEdit1Exit(Sender: TObject);
begin
        if DMirsaliye.CAltUstBilgiIBQ.Modified then
                DMirsaliye.CAltUstBilgiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.DBRichEdit2Enter(Sender: TObject);
begin
        DMirsaliye.CAltUstBilgiIBQ.Edit;
end;

procedure TFCikisIrsaliyesi.DBRichEdit2Exit(Sender: TObject);
begin
        if DMirsaliye.CAltUstBilgiIBQ.Modified then
                DMirsaliye.CAltUstBilgiIBQ.Post;
end;

procedure TFCikisIrsaliyesi.FontEdit1Accept(Sender: TObject);
begin
...
procedure TFCikisIrsaliyesi.FontEdit1BeforeExecute(Sender: TObject);
begin
        if ActiveControl is TDBRichEdit then
         begin
           if TDBRichEdit(ActiveControl).SelText <> '' then
...
procedure TFCikisIrsaliyesi.OnlineEdit10Change(Sender: TObject);
begin
        DMSabit.PersonelIBQ.Open;
