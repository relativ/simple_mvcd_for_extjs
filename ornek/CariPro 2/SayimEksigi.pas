...
Begin
        Result := FormatFloat(DMStokislem.SEBodroIBQCikisTUTAR.DisplayFormat, x);
end;

Procedure Datalari_Ac;
Begin
        if DMStokislem.SayimEksigiIBQ.Active = false then
        begin
                DMStokislem.SayimEksigiIBQ.Open;
                DMStokislem.SEBodroIBQ.Open;
...
procedure TFSayimEksigi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
        DMStokislem.SayimEksigiIBQ.Close;
        DMStokislem.SEBodroIBQ.Close;
        Action := caFree;
end;

procedure TFSayimEksigi.BFirstPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.SayimEksigiIBQ.First;
end;

procedure TFSayimEksigi.ButtonPreviousClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.SayimEksigiIBQ.Prior;
end;

procedure TFSayimEksigi.ButtonNextClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.SayimEksigiIBQ.Next;
end;

procedure TFSayimEksigi.BLastPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.SayimEksigiIBQ.Last;
end;

procedure TFSayimEksigi.BitBtn1Click(Sender: TObject);
begin
        PageControl1.ActivePage := TabSheet1;
        DMStokislem.SEBodroIBQ.DisableControls;
        Datalari_Ac;
        DMStokislem.SayimEksigiIBQ.Append;
        DMStokislem.SayimEksigiIBQ.Post;

        DMStokislem.SEBodroIBQ.EnableControls;
end;

procedure TFSayimEksigi.BitBtn2Click(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.RecordCount>0 Then
        begin
                if (DMStokislem.SayimEksigiIBQBELGENO.AsString = '') or (DMStokislem.SayimEksigiIBQBELGENO.AsFloat = 0) then
                begin
                        if Application.MessageBox('Bu Stok Ýþlemi silmek istediðinizden emin misiniz?',
                                '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
                        begin
                                DMStokislem.SayimEksigiIBQ.Edit;
                                DMStokislem.SayimEksigiIBQSISTEM.AsInteger := 2;
                                DMStokislem.SayimEksigiIBQ.Post;
                        end;
...
procedure TFSayimEksigi.BitBtn3Click(Sender: TObject);
begin
        Close;
end;

procedure TFSayimEksigi.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TFSayimEksigi.DBOnlineEdit4Change(Sender: TObject);
begin
        GroupBox6.Caption := ' Dövizli Tutar [ '+DMStokislem.SayimEksigiIBQC_PARABIRIMI.AsString +' ] ';
end;

procedure TFSayimEksigi.OnlineDBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
       if (DMStokislem.SEBodroIBQKAPATMA.AsInteger=2) and
                ((DMStokislem.SEBodroIBQCikisTUTAR.AsString<>'') or
                        (DMStokislem.SEBodroIBQCIKISTUTAR.AsString<>'') or
                        (DMStokislem.SEBodroIBQMIKTAR.AsString<>''))  Then
        Begin
                ...
                begin
                        ...
                        begin
                                if (Column.Field <> DMStokislem.SEBodroIBQCikisMIKTAR) and (Column.Field <> DMStokislem.SEBodroIBQCIKISMIKTAR) then
...
procedure TFSayimEksigi.Builemikapat1Click(Sender: TObject);
begin
        DMStokislem.SEBodroIBQ.edit;
        DMStokislem.SEBodroIBQKAPATMA.AsInteger:=2;
        DMStokislem.SEBodroIBQ.Post;
END;

procedure TFSayimEksigi.Busatilemia1Click(Sender: TObject);
begin
        DMStokislem.SEBodroIBQ.edit;
        DMStokislem.SEBodroIBQKAPATMA.AsInteger:=1;
        DMStokislem.SEBodroIBQ.Post;
end;

procedure TFSayimEksigi.OnlineDBGrid1ContextPopup(Sender: TObject;
  MousePos: TPoint; var Handled: Boolean);
begin
        if DMStokislem.SayimEksigiIBQCARIHESAPILISKI.AsString ='1' Then
        Begin
        if (DMStokislem.SEBodroIBQKAPATMA.AsInteger=1) Then
        Begin
        ...
        if (DMStokislem.SEBodroIBQCikisMIKTAR.AsFloat>0) or (DMStokislem.SEBodroIBQCikisTUTAR.AsFloat>0) Then
                SatrSil1.Visible:=True else SatrSil1.Visible:=False;

end;

procedure TFSayimEksigi.OnlineDBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
        if (Key=#13) Then
        Begin
                if (DMStokislem.SEBodroIBQ.Modified=True ) Then
                        DMStokislem.SEBodroIBQ.Post;
                OnlineDBGrid1.EditorMode :=True;
                if BarkodluCikis = True Then
                Begin
                        BarkodluCikis := False;
                        DMStokislem.SEBodroIBQ.Append;
                end;
        end;
...
procedure TFSayimEksigi.SatrEkle1Click(Sender: TObject);
begin
        DMStokislem.SEBodroIBQ.Append;
end;

procedure TFSayimEksigi.SatrSil1Click(Sender: TObject);
begin
        if DMStokislem.SEBodroIBQ.RecordCount>0 Then
        if Application.MessageBox('Bu satýrý silmek istediðinizden emin misiniz?',
                        '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
        Begin
                DMStokislem.SEBodroIBQ.Delete;
                if (DMStokislem.SEBodroIBQ.Modified=True ) Then
                        DMStokislem.SEBodroIBQ.Post;
        end;
end;

procedure TFSayimEksigi.StokEkle1Click(Sender: TObject);
begin
                ...
                begin
                        ...
                        FFiyatSorgu.DataSet := DMStokislem.SEBodroIBQ;
...
procedure TFSayimEksigi.BitBtn4Click(Sender: TObject);
begin   {
if DMStokislem.SayimEksigiIBQTAHSILAT.AsFloat>0 Then
Begin
        ...
        TQRLabel(FEvrak.Report.FindComponent('FaturaBasligi')).Caption := DMStokislem.SayimEksigiIBQCARIADI.AsString +' '+DMStokislem.SayimEksigiIBQCARISOYADI.AsString;
        if CariDM.CariKartIBQuery.Locate('CARIKOD',DMStokislem.SayimEksigiIBQCARIKOD.AsString,[]) Then
        Begin
...
procedure TFSayimEksigi.BitBtn5Click(Sender: TObject);
begin             {
if DMStokislem.SayimEksigiIBQBELGENO.AsInteger > 0 then
...
procedure TFSayimEksigi.HepsineStoaKaydet1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMStokislem.SEBodroIBQ.GetBookmark;
        DMProcedure.TumunuStokKaydetProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.SEBodroIBQISLEMKOD.AsInteger;
        ...
        DMStokislem.SEBodroIBQ.Close;
        DMStokislem.SEBodroIBQ.Open;
        DMStokislem.SEBodroIBQ.GotoBookmark(yer);

end;

procedure TFSayimEksigi.HibiriniStoakeydetme1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMStokislem.SEBodroIBQ.GetBookmark;
        DMProcedure.TumuStokSilProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.SEBodroIBQISLEMKOD.AsInteger;
...
procedure TFSayimEksigi.OnlineDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
        if Key = VK_F5 then
        begin
...
procedure TFSayimEksigi.lemSrasnaGreSrala1Click(Sender: TObject);
begin
        DMStokislem.SEBodroIBQ.SQL.Text := 'select * from STOKISLEM Where ISLEMKOD=:ISLEMKOD';
        DMStokislem.SEBodroIBQ.SQL.Add('Order by ISLEMKOD ASC');
        DMStokislem.SEBodroIBQ.Open;

end;

procedure TFSayimEksigi.FormDestroy(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
        DMStokislem.SayimEksigiIBQ.Close;
        DMStokislem.SEBodroIBQ.Close;
end;

procedure TFSayimEksigi.CariHesaplikisi1Click(Sender: TObject);
var
        yer: pointer;
begin
...
procedure TFSayimEksigi.DBOnlineCombo1KeyPress(Sender: TObject; var Key: Char);
begin
        if Key = #13 then
        begin
                Key := #0;
...
procedure TFSayimEksigi.DBOnlineCombo1Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
        DMStokislem.SayimEksigiIBQD_CikisTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.SayimEksigiIBQC_CARIPARABIRIM.AsString;
//        DMStokislem.SayimEksigiIBQD_TAHKDVTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.SayimEksigiIBQPARABIRIMI.AsString;
//        DMStokislem.SayimEksigiIBQD_TAHTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.SayimEksigiIBQPARABIRIMI.AsString;
        GroupBox6.Caption := ' Dövizli Tutar ['+DMStokislem.SayimEksigiIBQC_PARABIRIMI.AsString+']';
end;

procedure TFSayimEksigi.OnlineButton1Click(Sender: TObject);
begin
        ...
        FStokYeri.DataSet := DMStokislem.SayimEksigiIBQ;
        FStokYeri.BitBtn3.Visible := true;
        FStokYeri.Show;
end;

procedure TFSayimEksigi.DBOnlineEdit15Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
end;

procedure TFSayimEksigi.OnlineButton2Click(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Active then
        begin
                ...
                DMSabit.GGSecimIBQ.SQL.Add('Order by GELGITADI');
                DMSabit.GGSecimIBQ.Open;
                FGelGidSec.DataSet := DMStokislem.SayimEksigiIBQ;
                FGelGidSec.ShowModal;
        end;
end;

procedure TFSayimEksigi.DBOnlineEdit5Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
end;

procedure TFSayimEksigi.GroupBox3Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
end;

procedure TFSayimEksigi.OnlineDBGrid1Columns16KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMStokislem.SayimEksigiIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade);
                        TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade := '';
                        DMStokislem.SEBodroIBQ.Edit;
                        DMStokislem.SEBodroIBQVADE.AsDateTime := Vade;
                        DMStokislem.SEBodroIBQ.Post;
                end;
        end;
...
procedure TFSayimEksigi.OnlineDBGrid1Columns7EditButtonClick(
  Column: TOnlineColumn);
begin
        FParaBirimSecim.DataSet := DMStokislem.SEBodroIBQ;
        FParaBirimSecim.ShowModal;
end;

procedure TFSayimEksigi.GroupBox1Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified Then
                DMStokislem.SayimEksigiIBQ.Post;
end;

procedure TFSayimEksigi.OnlineDBCheckbox1Click(Sender: TObject);
begin
...
procedure TFSayimEksigi.DBOnlineLookupCombo1Click(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
        GroupBox6.Caption := ' Dövizli Tutar [ '+DMStokislem.SayimEksigiIBQC_PARABIRIMI.AsString +' ] ';
...
procedure TFSayimEksigi.DBOnlineLookupCombo1Exit(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Modified then
                DMStokislem.SayimEksigiIBQ.Post;
end;

procedure TFSayimEksigi.OnlineButton5Click(Sender: TObject);
var
        yer: pointer;
begin
        FKopyala.Show;
        DMProcedure.CeviriProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.SayimEksigiIBQISLEMKOD.AsInteger;
        ...
        DMStokislem.SayimEksigiIBQ.Open;
        DMStokislem.SEBodroIBQ.Open;
        DMStokislem.SayimEksigiIBQ.Locate('ISLEMKOD',DMProcedure.CeviriProc.ParamByName('ISLEMKOD3_IN').AsInteger,[]);
        DMData.IBTCariPro.CommitRetaining;
end;

procedure TFSayimEksigi.OnlineButton4Click(Sender: TObject);
begin
        if DMStokislem.SayimEksigiIBQ.Active then
        begin
                DMSabit.PersonelIBQ.Open;
                FPersonelSec.DataSet := DMStokislem.SayimEksigiIBQ;
...
procedure TFSayimEksigi.OnlineDBGrid1Columns17KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMStokislem.SEBodroIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[17]).Vade);
...
procedure TFSayimEksigi.OnlineButton14Click(Sender: TObject);
begin
...
procedure TFSayimEksigi.OnlineButton15Click(Sender: TObject);
begin
        FPersonelSec.DataSet := nil;
...
procedure TFSayimEksigi.OnlineButton10Click(Sender: TObject);
begin
        DMStokislem.SayimEksigiIBQ.DisableControls;
        DMStokislem.SayimEksigiIBQ.SQL.Text := 'select * from STOKISLEM where ISLEMTURKOD=40 AND SISTEM<>2';
        DMStokislem.SayimEksigiIBQ.SQL.Add('and TARIH >='+#39+DateToStr(DateTimePicker1.Date)+#39+' and TARIH <='+#39+DateToStr(DateTimePicker2.Date)+#39);
        if (OnlineEdit1.Text <> '') and (OnlineEdit2.Text <> '') then
                DMStokislem.SayimEksigiIBQ.SQL.Add('and STOKISLEMNO >='+OnlineEdit1.Text+' and STOKISLEMNO <='+OnlineEdit2.Text);
       if (OnlineEdit5.Text <> '') and (OnlineEdit6.Text <> '') then
                DMStokislem.SayimEksigiIBQ.SQL.Add('and BELGENO >='+OnlineEdit5.Text+' and BELGENO <='+OnlineEdit6.Text);
        if (OnlineEdit7.Text <> '') and (OnlineEdit8.Text <> '') then
                DMStokislem.SayimEksigiIBQ.SQL.Add('and EVRAKNO >='+OnlineEdit7.Text+' and EVRAKNO <='+OnlineEdit8.Text);
        if (OnlineEdit9.Text <> '') and (OnlineEdit10.Text <> '') then
                DMStokislem.SayimEksigiIBQ.SQL.Add('and PERSONELKOD >='+IntToStr(OnlineEdit9.Tag)+' and PERSONELKOD <='+IntToStr(OnlineEdit10.Tag));
        DMStokislem.SayimEksigiIBQ.Open;
        DMStokislem.SEBodroIBQ.Open;
        DMStokislem.SayimEksigiIBQ.EnableControls;
end;

procedure TFSayimEksigi.OnlineButton11Click(Sender: TObject);
begin
        OnlineEdit1.Text := '';
        OnlineEdit2.Text := '';
        OnlineEdit5.Text := '';
        ...
        OnlineEdit10.Tag := 0;
        DMStokislem.SayimEksigiIBQ.DisableControls;
        DMStokislem.SayimEksigiIBQ.SQL.Text := 'select * from STOKISLEM';
        DMStokislem.SayimEksigiIBQ.SQL.Add('where ISLEMTURKOD =40 AND SISTEM<>2');
...
procedure TFSayimEksigi.DBRichEdit1Enter(Sender: TObject);
begin
        DMStokislem.AltUstBilgiIBQ.Edit;
end;

procedure TFSayimEksigi.DBRichEdit1Exit(Sender: TObject);
begin
        if DMStokislem.AltUstBilgiIBQ.Modified then
                DMStokislem.AltUstBilgiIBQ.Post;
end;

procedure TFSayimEksigi.DBRichEdit2Enter(Sender: TObject);
begin
        DMStokislem.AltUstBilgiIBQ.Edit;
end;

procedure TFSayimEksigi.DBRichEdit2Exit(Sender: TObject);
begin
        if DMStokislem.AltUstBilgiIBQ.Modified then
                DMStokislem.AltUstBilgiIBQ.Post;
end;

procedure TFSayimEksigi.FontEdit1Accept(Sender: TObject);
begin
...
procedure TFSayimEksigi.FontEdit1BeforeExecute(Sender: TObject);
begin
        if ActiveControl is TDBRichEdit then
         begin
           if TDBRichEdit(ActiveControl).SelText <> '' then
...
procedure TFSayimEksigi.TabSheet2Show(Sender: TObject);
begin
        DateTimePicker1.DateTime := BugununTarihi;
        DateTimePicker2.DateTime := BugununTarihi;
end;

procedure TFSayimEksigi.FormShow(Sender: TObject);
begin
        DMStokislem.SayimEksigiIBQ.SQL.Text := 'select * from STOKISLEM';
        DMStokislem.SayimEksigiIBQ.SQL.Add('where ISLEMTURKOD =40 AND SISTEM<>2');
end;

procedure TFSayimEksigi.OnlineEdit9Change(Sender: TObject);
begin
        DMSabit.PersonelIBQ.Open;
...
procedure TFSayimEksigi.OnlineEdit10Change(Sender: TObject);
begin
        DMSabit.PersonelIBQ.Open;
