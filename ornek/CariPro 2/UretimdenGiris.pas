...
var
  FUretimdenGiris: TFUretimdenGiris;
...
Begin
        Result := FormatFloat(DMStokislem.UGBodroIBQGIRISTUTAR.DisplayFormat, x);
end;

Procedure Datalari_Ac;
Begin
        if DMStokislem.UretimGirisIBQ.Active = false then
        begin
                DMStokislem.UretimGirisIBQ.Open;
                DMStokislem.UGBodroIBQ.Open;
...
procedure TFUretimdenGiris.FormClose(Sender: TObject; var Action: TCloseAction);
begin
        DMStokislem.UretimGirisIBQ.Close;
        DMStokislem.UGBodroIBQ.Close;
        Action := caFree;
end;

procedure TFUretimdenGiris.BFirstPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.UretimGirisIBQ.First;
end;

procedure TFUretimdenGiris.ButtonPreviousClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.UretimGirisIBQ.Prior;
end;

procedure TFUretimdenGiris.ButtonNextClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.UretimGirisIBQ.Next;
end;

procedure TFUretimdenGiris.BLastPageClick(Sender: TObject);
begin
        Datalari_Ac;
        DMStokislem.UretimGirisIBQ.Last;
end;

procedure TFUretimdenGiris.BitBtn1Click(Sender: TObject);
begin
        PageControl1.ActivePage := TabSheet1;
        DMStokislem.UGBodroIBQ.DisableControls;
        Datalari_Ac;
        DMStokislem.UretimGirisIBQ.Append;
        DMStokislem.UretimGirisIBQ.Post;
        BitBtn8.SetFocus;
        DMStokislem.UGBodroIBQ.EnableControls;
end;

procedure TFUretimdenGiris.BitBtn2Click(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.RecordCount>0 Then
        begin
                if (DMStokislem.UretimGirisIBQBELGENO.AsString = '') or (DMStokislem.UretimGirisIBQBELGENO.AsFloat = 0) then
                begin
                        if Application.MessageBox('Bu Stok Ýþlemi silmek istediðinizden emin misiniz?',
                                '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
                        begin
                                DMStokislem.UretimGirisIBQ.Edit;
                                DMStokislem.UretimGirisIBQSISTEM.AsInteger := 2;
                                DMStokislem.UretimGirisIBQ.Post;
                        end;
...
procedure TFUretimdenGiris.BitBtn3Click(Sender: TObject);
begin
        Close;
end;

procedure TFUretimdenGiris.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) then
  begin
...
procedure TFUretimdenGiris.DBOnlineEdit4Change(Sender: TObject);
begin
        GroupBox6.Caption := ' Dövizli Tutar ['+DMStokislem.UretimGirisIBQC_PARABIRIMI.AsString +' ] ';
end;

procedure TFUretimdenGiris.OnlineDBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
       if (DMStokislem.UGBodroIBQKAPATMA.AsInteger=2) and
                ((DMStokislem.UGBodroIBQGIRISTUTAR.AsString<>'') or
                        (DMStokislem.UGBodroIBQCIKISTUTAR.AsString<>'') or
                        (DMStokislem.UGBodroIBQMIKTAR.AsString<>''))  Then
        Begin
                ...
                begin
                        ...
                        begin
                                if (Column.Field <> DMStokislem.UGBodroIBQGIRISMIKTAR) and (Column.Field <> DMStokislem.UGBodroIBQCIKISMIKTAR) then
...
procedure TFUretimdenGiris.Builemikapat1Click(Sender: TObject);
begin
        DMStokislem.UGBodroIBQ.edit;
        DMStokislem.UGBodroIBQKAPATMA.AsInteger:=2;
        DMStokislem.UGBodroIBQ.Post;
END;

procedure TFUretimdenGiris.Busatilemia1Click(Sender: TObject);
begin
        DMStokislem.UGBodroIBQ.edit;
        DMStokislem.UGBodroIBQKAPATMA.AsInteger:=1;
        DMStokislem.UGBodroIBQ.Post;
end;

procedure TFUretimdenGiris.OnlineDBGrid1ContextPopup(Sender: TObject;
  MousePos: TPoint; var Handled: Boolean);
begin
        if DMStokislem.UretimGirisIBQCARIHESAPILISKI.AsString ='1' Then
        Begin
        if (DMStokislem.UGBodroIBQKAPATMA.AsInteger=1) Then
        Begin
        ...
        if (DMStokislem.UGBodroIBQGIRISMIKTAR.AsFloat>0) or (DMStokislem.UGBodroIBQGIRISTUTAR.AsFloat>0) Then
                SatrSil1.Visible:=True else SatrSil1.Visible:=False;

end;

procedure TFUretimdenGiris.OnlineDBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
        if (Key=#13) Then
        Begin
                if (DMStokislem.UGBodroIBQ.Modified=True ) Then
                        DMStokislem.UGBodroIBQ.Post;
                OnlineDBGrid1.EditorMode :=True;
                if BarkodluGiris = True Then
                Begin
                        BarkodluGiris := False;
                        DMStokislem.UGBodroIBQ.Append;
                end;
        end;
...
procedure TFUretimdenGiris.SatrEkle1Click(Sender: TObject);
begin
        DMStokislem.UGBodroIBQ.Append;
end;

procedure TFUretimdenGiris.SatrSil1Click(Sender: TObject);
begin
        if DMStokislem.UGBodroIBQ.RecordCount>0 Then
        if Application.MessageBox('Bu satýrý silmek istediðinizden emin misiniz?',
                        '          Uyarý Mesajý', mb_YesNo+mb_DefButton2)=6 Then
        Begin
                DMStokislem.UGBodroIBQ.Delete;
                if (DMStokislem.UGBodroIBQ.Modified=True ) Then
                        DMStokislem.UGBodroIBQ.Post;
        end;
end;

procedure TFUretimdenGiris.BitBtn8Click(Sender: TObject);
begin
        if (DMStokislem.UretimGirisIBQ.Active) and (DMStokislem.UGBodroIBQ.Active) then
        begin
               FCariSecim.DataSet := DMStokislem.UretimGirisIBQ;
               FCariSecim.ShowModal;
        end else MessageBox(handle,'Yeni iþlem yapmadan cari seçemezsiniz.','Uyarý',MB_OK);
end;

procedure TFUretimdenGiris.StokEkle1Click(Sender: TObject);
begin
                ...
                begin
                        ...
                        FFiyatSorgu.DataSet := DMStokislem.UGBodroIBQ;
...
procedure TFUretimdenGiris.BitBtn4Click(Sender: TObject);
begin   {
if DMStokislem.UretimGirisIBQTAHSILAT.AsFloat>0 Then
Begin
        ...
        TQRLabel(FEvrak.Report.FindComponent('FaturaBasligi')).Caption := DMStokislem.UretimGirisIBQCARIADI.AsString +' '+DMStokislem.UretimGirisIBQCARISOYADI.AsString;
        if CariDM.CariKartIBQuery.Locate('CARIKOD',DMStokislem.UretimGirisIBQCARIKOD.AsString,[]) Then
        Begin
...
procedure TFUretimdenGiris.BitBtn5Click(Sender: TObject);
begin             {
if DMStokislem.UretimGirisIBQBELGENO.AsInteger > 0 then
...
procedure TFUretimdenGiris.HepsineStoaKaydet1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMStokislem.UGBodroIBQ.GetBookmark;
        DMProcedure.TumunuStokKaydetProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.UGBodroIBQISLEMKOD.AsInteger;
        ...
        DMStokislem.UGBodroIBQ.Close;
        DMStokislem.UGBodroIBQ.Open;
        DMStokislem.UGBodroIBQ.GotoBookmark(yer);

end;

procedure TFUretimdenGiris.HibiriniStoakeydetme1Click(Sender: TObject);
var
        yer: pointer;
begin
        yer := DMStokislem.UGBodroIBQ.GetBookmark;
        DMProcedure.TumuStokSilProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.UGBodroIBQISLEMKOD.AsInteger;
...
procedure TFUretimdenGiris.OnlineDBGrid2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
        if Key = VK_F5 then
        begin
...
procedure TFUretimdenGiris.lemSrasnaGreSrala1Click(Sender: TObject);
begin
        DMStokislem.UGBodroIBQ.SQL.Text := 'select * from STOKISLEM Where ISLEMKOD=:ISLEMKOD';
        DMStokislem.UGBodroIBQ.SQL.Add('Order by ISLEMKOD ASC');
        DMStokislem.UGBodroIBQ.Open;

end;

procedure TFUretimdenGiris.FormDestroy(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
        DMStokislem.UretimGirisIBQ.Close;
        DMStokislem.UGBodroIBQ.Close;
end;

procedure TFUretimdenGiris.CariHesaplikisi1Click(Sender: TObject);
var
        yer: pointer;
begin
...
procedure TFUretimdenGiris.DBOnlineCombo1KeyPress(Sender: TObject; var Key: Char);
begin
        if Key = #13 then
        begin
                Key := #0;
...
procedure TFUretimdenGiris.DBOnlineCombo1Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
        DMStokislem.UretimGirisIBQD_GIRISTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.UretimGirisIBQC_CARIPARABIRIM.AsString;
//        DMStokislem.UretimGirisIBQD_TAHKDVTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.UretimGirisIBQPARABIRIMI.AsString;
//        DMStokislem.UretimGirisIBQD_TAHTUTAR.DisplayFormat := '###,###.0 '+ DMStokislem.UretimGirisIBQPARABIRIMI.AsString;
        GroupBox6.Caption := ' Dövizli Tutar ['+DMStokislem.UretimGirisIBQC_PARABIRIMI.AsString+']';
end;

procedure TFUretimdenGiris.OnlineButton1Click(Sender: TObject);
begin
        ...
        FStokYeri.DataSet := DMStokislem.UretimGirisIBQ;
        FStokYeri.BitBtn3.Visible := true;
        FStokYeri.Show;
end;

procedure TFUretimdenGiris.DBOnlineEdit15Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
end;

procedure TFUretimdenGiris.OnlineButton2Click(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Active then
        begin
                ...
                DMSabit.GGSecimIBQ.SQL.Add('Order by GELGITADI');
                DMSabit.GGSecimIBQ.Open;
                FGelGidSec.DataSet := DMStokislem.UretimGirisIBQ;
                FGelGidSec.ShowModal;
        end;
end;

procedure TFUretimdenGiris.DBOnlineEdit5Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
end;

procedure TFUretimdenGiris.DBOnlineEdit7Change(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQC_CARIADI.AsString<>'' Then
                Panel2.Caption := 'Stok Giriþi [ '+DMStokislem.UretimGirisIBQC_CARIADI.AsString + ' ]'
        else Panel2.Caption := 'Stok Giriþi';
end;

procedure TFUretimdenGiris.GroupBox3Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
end;

procedure TFUretimdenGiris.OnlineDBGrid1Columns16KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMStokislem.UretimGirisIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade);
                        TOnlineColumn(OnlineDBGrid1.Columns[16]).Vade := '';
                        DMStokislem.UGBodroIBQ.Edit;
                        DMStokislem.UGBodroIBQVADE.AsDateTime := Vade;
                        DMStokislem.UGBodroIBQ.Post;
                end;
        end;
...
procedure TFUretimdenGiris.OnlineButton3Click(Sender: TObject);
begin
        if (DMStokislem.UretimGirisIBQCARIKOD.AsInteger > 0) or (DMStokislem.UretimGirisIBQCARINO.AsInteger > 0) then
        ...
        begin
                DMStokislem.UretimGirisIBQ.Edit;
                DMStokislem.UretimGirisIBQCARIKOD.AsInteger:=0;
                DMStokislem.UretimGirisIBQCARINO.AsInteger := 0;
                DMStokislem.UretimGirisIBQ.Post;
        end;
end;

procedure TFUretimdenGiris.OnlineDBGrid1Columns7EditButtonClick(
  Column: TOnlineColumn);
begin
        FParaBirimSecim.DataSet := DMStokislem.UGBodroIBQ;
        FParaBirimSecim.ShowModal;
end;

procedure TFUretimdenGiris.GroupBox1Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified Then
                DMStokislem.UretimGirisIBQ.Post;
end;

procedure TFUretimdenGiris.OnlineDBCheckbox1Click(Sender: TObject);
begin
...
procedure TFUretimdenGiris.DBOnlineLookupCombo1Click(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
        GroupBox6.Caption := ' Dövizli Tutar ['+DMStokislem.UretimGirisIBQC_PARABIRIMI.AsString +' ] ';
...
procedure TFUretimdenGiris.DBOnlineLookupCombo1Exit(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Modified then
                DMStokislem.UretimGirisIBQ.Post;
end;

procedure TFUretimdenGiris.OnlineButton5Click(Sender: TObject);
var
        yer: pointer;
begin
        FKopyala.Show;
        DMProcedure.CeviriProc.ParamByName('ISLEMKOD_IN').AsInteger := DMStokislem.UretimGirisIBQISLEMKOD.AsInteger;
        ...
        DMStokislem.UretimGirisIBQ.Open;
        DMStokislem.UGBodroIBQ.Open;
        DMStokislem.UretimGirisIBQ.Locate('ISLEMKOD',DMProcedure.CeviriProc.ParamByName('ISLEMKOD3_IN').AsInteger,[]);
        DMData.IBTCariPro.CommitRetaining;
end;

procedure TFUretimdenGiris.OnlineButton4Click(Sender: TObject);
begin
        if DMStokislem.UretimGirisIBQ.Active then
        begin
                DMSabit.PersonelIBQ.Open;
                FPersonelSec.DataSet := DMStokislem.UretimGirisIBQ;
...
procedure TFUretimdenGiris.OnlineButton9Click(Sender: TObject);
var
        TopVade,TopFiyat,OrtVade: real;
begin
        if (DMStokislem.UGBodroIBQ.Active = true) and (DMStokislem.UGBodroIBQ.RecordCount > 0) then
        begin
                DMStokislem.UGBodroIBQ.First;
                while not DMStokislem.UGBodroIBQ.Eof do
                begin
                        TopVade := TopVade + (DMStokislem.UGBodroIBQGIRISTUTAR.AsFloat * (DMStokislem.UGBodroIBQVADE.AsDateTime - DMStokislem.UretimGirisIBQTARIH.AsDateTime));
                        TopFiyat := TopFiyat + DMStokislem.UGBodroIBQGIRISTUTAR.AsFloat;
                        DMStokislem.UGBodroIBQ.Next;
                end;
                OrtVade := TopVade / TopFiyat;
                OrtVade := OrtVade + (DMStokislem.UretimGirisIBQTARIH.AsFloat) ;
                DMStokislem.UretimGirisIBQ.Edit;
                DMStokislem.UretimGirisIBQVADE.AsString := DateToStr((OrtVade));
                DMStokislem.UretimGirisIBQ.Post;
        end;
...
begin
        ...
        DMStokislem.UGBodroIBQ.DisableControls;
        DMStokislem.UGBodroIBQ.First;
        for i := 0 to DMStokislem.UGBodroIBQ.RecordCount do
        begin
      //  while not DMStokislem.UGBodroIBQ.Eof do
       // begin
                ToplamVade := ToplamVade + DMStokislem.UGBodroIBQVADE.AsDateTime;
        ...
        DMStokislem.UretimGirisIBQ.Edit;
        DMStokislem.UretimGirisIBQVADE.AsString :=DateToStr(OrtVadeTarih);
        DMStokislem.UretimGirisIBQ.Post;

               }
end;

procedure TFUretimdenGiris.OnlineDBGrid1Columns17KeyPress(Column: TOnlineColumn;
  var Key: Char);
var
        Vade: TDateTime;
begin
        ...
        begin
                ...
                begin
                        Vade := DMStokislem.UGBodroIBQTARIH.AsDateTime+StrToInt(TOnlineColumn(OnlineDBGrid1.Columns[17]).Vade);
...
procedure TFUretimdenGiris.OnlineButton12Click(Sender: TObject);
begin
...
procedure TFUretimdenGiris.OnlineButton13Click(Sender: TObject);
begin
        FCariSecim.DataSet := nil;
...
procedure TFUretimdenGiris.OnlineButton14Click(Sender: TObject);
begin
...
procedure TFUretimdenGiris.OnlineButton15Click(Sender: TObject);
begin
        FPersonelSec.DataSet := nil;
...
procedure TFUretimdenGiris.OnlineButton10Click(Sender: TObject);
begin
        DMStokislem.UretimGirisIBQ.DisableControls;
        DMStokislem.UretimGirisIBQ.SQL.Text := 'select * from STOKISLEM where ISLEMTURKOD=37 AND SISTEM<>2';
        DMStokislem.UretimGirisIBQ.SQL.Add('and TARIH >='+#39+DateToStr(DateTimePicker1.Date)+#39+' and TARIH <='+#39+DateToStr(DateTimePicker2.Date)+#39);
        if (OnlineEdit1.Text <> '') and (OnlineEdit2.Text <> '') then
                DMStokislem.UretimGirisIBQ.SQL.Add('and STOKISLEMNO >='+OnlineEdit1.Text+' and STOKISLEMNO <='+OnlineEdit2.Text);
        if (OnlineEdit3.Text <> '') and (OnlineEdit4.Text <> '') then
                DMStokislem.UretimGirisIBQ.SQL.Add('and CARINO >='+IntToStr(OnlineEdit3.Tag)+' and CARINO <='+IntToStr(OnlineEdit4.Tag));
        if (OnlineEdit5.Text <> '') and (OnlineEdit6.Text <> '') then
                DMStokislem.UretimGirisIBQ.SQL.Add('and BELGENO >='+OnlineEdit5.Text+' and BELGENO <='+OnlineEdit6.Text);
        if (OnlineEdit7.Text <> '') and (OnlineEdit8.Text <> '') then
                DMStokislem.UretimGirisIBQ.SQL.Add('and EVRAKNO >='+OnlineEdit7.Text+' and EVRAKNO <='+OnlineEdit8.Text);
        if (OnlineEdit9.Text <> '') and (OnlineEdit10.Text <> '') then
                DMStokislem.UretimGirisIBQ.SQL.Add('and PERSONELKOD >='+IntToStr(OnlineEdit9.Tag)+' and PERSONELKOD <='+IntToStr(OnlineEdit10.Tag));
        DMStokislem.UretimGirisIBQ.Open;
        DMStokislem.UGBodroIBQ.Open;
        DMStokislem.UretimGirisIBQ.EnableControls;
end;

procedure TFUretimdenGiris.OnlineButton11Click(Sender: TObject);
begin
        OnlineEdit1.Text := '';
        OnlineEdit2.Text := '';
        OnlineEdit3.Text := '';
        ...
        OnlineEdit10.Tag := 0;
        DMStokislem.UretimGirisIBQ.DisableControls;
        DMStokislem.UretimGirisIBQ.SQL.Text := 'select * from STOKISLEM';
        DMStokislem.UretimGirisIBQ.SQL.Add('where ISLEMTURKOD =37 AND SISTEM<>2');
        DMStokislem.UretimGirisIBQ.Open;
        DMStokislem.UretimGirisIBQ.EnableControls;
        
end;

procedure TFUretimdenGiris.DBRichEdit1Enter(Sender: TObject);
begin
        DMStokislem.AltUstBilgiIBQ.Edit;
end;

procedure TFUretimdenGiris.DBRichEdit1Exit(Sender: TObject);
begin
        if DMStokislem.AltUstBilgiIBQ.Modified then
                DMStokislem.AltUstBilgiIBQ.Post;
end;

procedure TFUretimdenGiris.DBRichEdit2Enter(Sender: TObject);
begin
        DMStokislem.AltUstBilgiIBQ.Edit;
end;

procedure TFUretimdenGiris.DBRichEdit2Exit(Sender: TObject);
begin
        if DMStokislem.AltUstBilgiIBQ.Modified then
                DMStokislem.AltUstBilgiIBQ.Post;
end;

procedure TFUretimdenGiris.FontEdit1Accept(Sender: TObject);
begin
...
procedure TFUretimdenGiris.FontEdit1BeforeExecute(Sender: TObject);
begin
        if ActiveControl is TDBRichEdit then
         begin
           if TDBRichEdit(ActiveControl).SelText <> '' then
...
procedure TFUretimdenGiris.TabSheet2Show(Sender: TObject);
begin
        DateTimePicker1.DateTime := BugununTarihi;
        DateTimePicker2.DateTime := BugununTarihi;
end;

procedure TFUretimdenGiris.FormShow(Sender: TObject);
begin
        DMStokislem.UretimGirisIBQ.SQL.Text := 'select * from STOKISLEM';
        DMStokislem.UretimGirisIBQ.SQL.Add('where ISLEMTURKOD =39 AND SISTEM<>2');
end;

procedure TFUretimdenGiris.OnlineEdit3Change(Sender: TObject);
begin
        if DMCari.CariKartIBQ.Locate('CARINO',OnlineEdit3.Text,[]) then
                OnlineEdit3.Tag := DMCari.CariKartIBQCARIKOD.AsInteger;
end;

procedure TFUretimdenGiris.OnlineEdit4Change(Sender: TObject);
begin
        if DMCari.CariKartIBQ.Locate('CARINO',OnlineEdit4.Text,[]) then
                OnlineEdit4.Tag := DMCari.CariKartIBQCARIKOD.AsInteger;
end;

procedure TFUretimdenGiris.OnlineEdit9Change(Sender: TObject);
begin
        DMSabit.PersonelIBQ.Open;
...
procedure TFUretimdenGiris.OnlineEdit10Change(Sender: TObject);
begin
        DMSabit.PersonelIBQ.Open;
