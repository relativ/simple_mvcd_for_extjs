...
type
  TdlgReportProps = class(TForm)
    Button1: TOnlineButton;
    Button2: TOnlineButton;
    GroupBox1: TGroupBox;
    ComboBox1: TComboBox;
    Label1: TLabel;
    Edit1: TEdit;
    ...
    GroupBox4: TGroupBox;
    Panel1: TPanel;
    Button3: TOnlineButton;
    CheckBox1: TOnlineCheckbox;
    CheckBox2: TOnlineCheckbox;
...
begin
  if ComboBox1.ItemIndex = -1 then
    raise Exception.Create('Yanlýþ Kaðýt Türü');
  if (ComboBox1.ItemIndex = 7) and not _CanConvertToFloat(Edit1.Text) then
    raise Exception.Create('Sayfa Uzunluðu Yanlýþ');
  if (ComboBox1.ItemIndex = 7) and not _CanConvertToFloat(Edit2.Text) then
    raise Exception.Create('Sayfa Geniþliði Yanlýþ');
  if ComboBox2.ItemIndex = -1 then
    raise Exception.Create('Sayfa Yönlendirmesi Yanlýþ');
  if not _CanConvertToFloat(Edit3.Text) then
    raise Exception.Create('Sol Boþluk Yanlýþ');
  if not _CanConvertToFloat(Edit4.Text) then
    raise Exception.Create('Sað Boþluk Yanlýþ');
  if not _CanConvertToFloat(Edit5.Text) then
    raise Exception.Create('Üst Boþluk Yanlýþ');
  if not _CanConvertToFloat(Edit6.Text) then
    raise Exception.Create('Alt Boþluk Yanlýþ');
  if not _CanConvertToFloat(Edit7.Text) then
    raise Exception.Create('Yanlýs Satýr Boþluðu');
  if ComboBox3.ItemIndex = -1 then
    raise Exception.Create('Yanlýþ Ünite Türü');
end;
...
procedure TdlgReportProps.FormDestroy(Sender: TObject);
begin
        FReport.Free;
end;
