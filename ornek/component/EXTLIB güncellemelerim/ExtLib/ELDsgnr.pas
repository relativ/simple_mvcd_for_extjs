...
uses
  ...
  Dialogs, ExtCtrls, Math, StdCtrls, Menus, ELSConsts
...
type
  ...
  private
    ...
    procedure SetXStep(const Value: Integer);
    procedure SetYStep(const Value: Integer);
    procedure SetColor(const Value: TColor);
  public
    constructor Create(AOwnerDesigner: TELCustomDesigner);
    procedure Assign(Source: TPersistent); override;
  published
    property XStep: Integer read FXStep write SetXStep default 8;
    property YStep: Integer read FYStep write SetYStep default 8;
  ...
  public
    ...
    procedure GetCotnrols(AResult: TList);
...
const
  ...
  DESIGNER_SIZING  = DESIGNER_BASE + $002; // TDMSizing
  DESIGNER_CANCEL  = DESIGNER_BASE + $003; // Params not used
...
begin
  Result := False;
 // if Sender <> FForm then Sender := GetDesignControl(Sender);
...
begin
  ...
  LR := FForm.Canvas.ClipRect;
  LI := ((LR.Left + FForm.HorzScrollBar.ScrollPos) div FGrid.FXStep) *
    FGrid.FXStep - FForm.HorzScrollBar.ScrollPos;
  if LI < LR.Left then Inc(LI, FGrid.FXStep);
  LMinJ := ((LR.Top + FForm.VertScrollBar.ScrollPos) div FGrid.FYStep) *
    FGrid.FYStep - FForm.VertScrollBar.ScrollPos;
  if LMinJ < LR.Top then Inc(LMinJ, FGrid.FYStep);
  while LI <= LR.Right do
  begin
    LJ := LMinJ;
    while LJ <= LR.Bottom do
    begin
      FForm.Canvas.Pixels[LI, LJ] := FGrid.Color;
      Inc(LJ, FGrid.FYStep);
    end;
    Inc(LI, FGrid.FXStep);
  end;
...
begin
  { Determine is message design or not }
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Sender := GetDesignControl(Sender);
              while (Sender <> nil) and (not (Sender is TWinControl) or
                not (csAcceptsControls in Sender.ControlStyle)) do
                Sender := Sender.Parent;
              if (Sender <> nil) and Result and (Message.Msg = WM_LBUTTONDOWN) then
                _InitDrawMode(TWinControl(Sender), SelectRectProc, False, False,
                False, False);
            end
            else
            begin
              ...
              try
                ...
                begin
                  ...
                  FSelCtrls.GetCotnrols(LControls);
                end;
      ...
      case TWMKeyDown(Message).CharCode of
        VK_ESCAPE:
          ...
          begin
            LDMCancelMessage.Msg := DESIGNER_CANCEL;
            MouseMessage(FForm, LDMCancelMessage);
          end else FSelCtrls.ClearExcept(FRoot);
...
procedure TELDesignerGrid.SetXStep(const Value: Integer);
begin
...
procedure TELDesignerGrid.SetYStep(const Value: Integer);
begin
...
begin
  ...
  begin
    if (ASender is TWinControl) and
      (csAcceptsControls in ASender.ControlStyle){ and
      IsDesignControl(ASender) }then
    begin
...
begin
  ...
  try
    ...
    if Count = 0 then
    begin
      if AControl <> nil then Add(AControl);
    end
    else
...
begin
  if Assigned(OnControlHint) then OnControlHint(Self, AControl, AHint);
end;
...
begin
  if Assigned(OnControlInserting) then OnControlInserting(Self, AControlClass);
end;
...
begin
  ...
  try
    ...
    for LI := 0 to AControls.Count - 1 do Add(AControls[LI]);
  finally
    EndUpdate;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      try
        LControls := TList.Create;
        try
          SelectedControls.GetCotnrols(LControls);
...
procedure TELDesignerSelectedControls.GetCotnrols(AResult: TList);
var
  LI: Integer;
begin
  AResult.Clear;
