...
type
  ...
  bcCodeUPC_E0,
  bcCodeUPC_E1,
  bcCodeUPC_Supp2,    { UPC 2 }
  bcCodeUPC_Supp5,    { UPC 5  }
  bcCodeEAN128A,
  bcCodeEAN128B,
  bcCodeEAN128C
  );
...
constructor TTechBarcode.Create(Owner:TComponent);
begin
...
destructor TTechBarcode.Destroy;
begin
  FShowTextFont.Free;
  inherited;
end;

procedure TTechBarcode.Assign(Source: TPersistent);
var
   BSource : TTechBarcode;
begin
   if Source is TTechBarcode then
   begin
      BSource    := TTechBarcode(Source);
...
function TTechBarcode.GetTypText:string;
begin
...
{ set Modul Width  }
procedure TTechBarcode.SetModul(v:integer);
begin
...
procedure TTechBarcode.OneBarProps(code:char; var Width:integer; var lt:TBarLineType);
begin
  case code of
...
function TTechBarcode.MakeData : string;
var
  i : integer;
begin
  ...
  case Typ of
    ...
    raise Exception.CreateFmt('%s: uyumsuz barkod türü', [self.ClassName]);
  end;
...
function TTechBarcode.GetWidth:integer;
var
  data : string;
  i : integer;
  w : integer;
...
procedure TTechBarcode.SetWidth(Value :integer);
var
  data : string;
  i : integer;
  w, wtotal : integer;
...
function TTechBarcode.DoCheckSumming(const data : string):string;
begin
...
function TTechBarcode.Code_EAN8:string;
var
  i : integer;
  tmp : String;
begin
...
function TTechBarcode.Code_EAN13:string;
var
  i, LK: integer;
  tmp : String;
begin
...
{$IFDEF ASSERT_SUPPORTED}
  Assert(Length(tmp) = 13, 'Barkod uzunluðu uyumsuz (EAN13)');
{$ENDIF}
...
function TTechBarcode.Code_2_5_interleaved:string;
var
  i, j: integer;
  c : char;
...
function TTechBarcode.Code_2_5_industrial:string;
var
  i, j: integer;
begin
...
function TTechBarcode.Code_2_5_matrix:string;
var
  i, j: integer;
  c :char;
begin
...
function TTechBarcode.Code_39:string;
...
function TTechBarcode.Code_39Extended:string;
...
function TTechBarcode.Code_128:string;
type TCode128 =
  record
    a, b : char;
    c : string[2];
...
begin
  case FTyp of
    ...
    bcCode128C, bcCodeEAN128C:
      ...
      raise Exception.CreateFmt('%s: Kod_128 için barkod türü uyumsuz.', [self.ClassName]);
  end;
...
function TTechBarcode.Code_93:string;
type TCode93 =
  record
    c : char;
...
begin
  ...
  begin
    ...
    if idx < 0 then
      raise Exception.CreateFmt('%s:Code93 bozuk data <%s>', [self.ClassName,FText]);
...
function TTechBarcode.Code_93Extended:string;
...
function TTechBarcode.Code_MSI:string;
...
function TTechBarcode.Code_PostNet:string;
...
function TTechBarcode.Code_Codabar:string;
type TCodabar =
  record
    c : char;
...
{Assist function}
function TTechBarcode.SetLen(pI:byte):string;
begin
...
function TTechBarcode.Code_UPC_A:string;
var
  i : integer;
  tmp : String;
begin
...
function TTechBarcode.Code_UPC_E0:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TTechBarcode.Code_UPC_E1:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TTechBarcode.Code_Supp5:string;
var i,j : integer;
   tmp : String;
   c   : char;
begin
...
function TTechBarcode.Code_Supp2:string;
var i,j : integer;
   tmp,mS : String;
begin
  FText := SetLen(2);
...
procedure TTechBarcode.MakeModules;
begin
  case Typ of
    bcCode_2_5_interleaved,
    bcCode_2_5_industrial,
...
procedure TTechBarcode.DoLines(data:string; Canvas:TCanvas);
...
procedure TTechBarcode.DrawBarcode(Canvas:TCanvas);
var
  data : string;
  SaveFont: TFont;
  SavePen: TPen;
...
procedure TTechBarcode.DrawText(Canvas:TCanvas);
var
  PosX, PosY: Integer;
  SaveFont: TFont;
begin
...
procedure TTechBarcode.DoChange;
begin
   if Assigned(FOnChange) then
      FOnChange(Self);
end;

procedure TTechBarcode.SetRatio(const Value: Double);
begin
...
procedure TTechBarcode.SetTyp(const Value: TBarcodeType);
begin
   if Value <> FTyp then
   begin
      FTyp := Value;
...
procedure TTechBarcode.SetAngle(const Value: Double);
begin
...
procedure TTechBarcode.SetText(const Value: string);
begin
   if Value <> FText then
   begin
      FText := Value;
...
procedure TTechBarcode.SetShowText(const Value: TBarcodeOption);
begin
...
procedure TTechBarcode.SetTop(const Value: Integer);
begin
   if Value <> FTop then
   begin
      FTop := Value;
...
procedure TTechBarcode.SetLeft(const Value: Integer);
begin
   if Value <> FLeft then
   begin
      FLeft := Value;
...
procedure TTechBarcode.SetCheckSum(const Value: Boolean);
begin
...
procedure TTechBarcode.SetHeight(const Value: integer);
begin
...
function TTechBarcode.GetCanvasHeight: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
  Result := Round(abs(sin(alpha))*Self.Width + abs(cos(alpha))*Self.Height + 0.5);
end;

function TTechBarcode.GetCanvasWidth: Integer;
var
  alpha :Extended;
begin
  alpha := FAngle/180.0*pi;
  Result := Round(abs(cos(alpha))*Self.Width + abs(sin(alpha))*Self.Height + 0.5);
end;


procedure TTechBarcode.SetShowTextFont(const Value: TFont);
begin
  FShowTextFont.Assign(Value);
  DoChange;
end;

procedure TTechBarcode.SetShowTextPosition(const Value: TShowTextPosition);
begin
