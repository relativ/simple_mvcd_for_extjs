...
uses
 ...
 const
  TechDBDateEdit_ABOUT = 'Tech Yazýlým / Serkan Güneþ'#13#10+'E-Mail: Tech@Techbilgisayar.com. '#13#10+'Web: www.Techbilgisayar.com';

type
  TTechDBDateEdit = class(TDBEdit)
  private
...
procedure Register;
begin
  RegisterComponents('TechDB', [TTechDBDateEdit]);
end;

constructor TTechDBDateEdit.Create(aOwner: TComponent);
begin
...
procedure TTechDBDateEdit.OtoTarihYaz(const Val: Boolean);
begin
        OtoTarih1 := Val;
...
procedure TTechDBDateEdit.KeyPress(var Key: Char);
var
        tarih1: string;
begin
        inherited;
...
procedure TTechDBDateEdit.Change;
begin
        inherited Change;
        if OtoTarih1 = true then
        begin
...
function TTechDBDateEdit.GetVersion: string;
begin
  Result := TechDBDateEdit_ABOUT;
end;

procedure TTechDBDateEdit.SetVersion(const Val: string);
begin
  { Kayýt giriþi yapýlamaz }
end;

procedure TTechDBDateEdit.CMEnter(var Message: TCMEnter); //kutucuða girdiðinde
begin
inherited;
        Color := color1;
        Refresh;
end;
procedure TTechDBDateEdit.CMExit(var Message: TCMExit); // kutucuktan çýktýðýnda
var
        Ay: word;
        gun: word;
        yil: word;
...
procedure TTechDBDateEdit.SetGirisColor(gcolor: TColor);
begin
...
procedure TTechDBDateEdit.SetCikisColor(ccolor: TColor);
begin
